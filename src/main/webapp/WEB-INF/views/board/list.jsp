...
$(function(){
	$("#btnWrite").click(function(){
		location.href="/spring04/board/write.do";
	});
});
function list(page){
	location.href="/spring04/board/list.do?curPage="+page+"&search_option=${map.search_option}&keyword=${map.keyword}";
}
</script>
</head>
<body>
...
<h2>게시판</h2>
<form name="form1" method="post" action="/spring04/board/list.do">
...
<c:forEach var="row" items="${map.list}">
	<tr>
		<td>${row.idx}</td>
		<td><a href="/spring04/board/detail.do?idx=${row.idx}&cur_page=${map.page_info.curPage}&search_option=${map.search_option}&keyword=${map.keyword}">${row.title}</a>
			<c:if test="${row.cnt > 0 }">
	...
	<tr>
		<td colspan="5" align="center">
			...
			<c:if test="${map.page_info.curBlock > 1 }">
				<a href="#" onclick="list('${map.page_info.prevPage}')">[이전]</a>
			</c:if>			
			...
			<c:if test="${map.page_info.curBlock <= map.page_info.totBlock}">
				<a href="#" onclick="list('${map.page_info.nextPage}')">[다음]</a>
			</c:if>
			<c:if test="${map.page_info.curBlock <= map.page_info.totPage}">
				<a href="#" onclick="list('${map.page_info.totPage}')">[끝]</a>
			</c:if>			
		</td>
	</tr>
</table>
